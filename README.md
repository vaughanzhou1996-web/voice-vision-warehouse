...
传统库存系统只能回答"**现在缺什么**"——发现缺货时往往已经晚了。本系统的核心差异是**打通内部数据库，回答"哪天会断料"**：

```
船舶建造周期表（哪天安装哪台设备）
        ↓ 设备 → 备件 BOM 映射
预期出库计划（备件 × 数量 × 日期）
        ↓ 叠加实时库存（PostgreSQL 出入库流水）
库存水位曲线（未来 60 天逐日推算）
        ↓
断料日 + 最晚到货日 + 三档风险灯
```

**完整链路**：`data/build-schedule.json` 定义建造节点与设备安装计划 → `lib/forecast.js` 把每台设备映射到所需备件，生成预期出库 → 与 PostgreSQL 实时库存逐日轧差，推算每个备件的水位阶梯曲线 → 水位首次跌破安全线（3件）的日期即**断料日**，倒推物流周期得**最晚到货日** → AI 洞察用人话输出决策建议：

> "2026-08-10 系泊试验需船用断路器 4 只，当前仅 2 只，且 08-05 分段制造先用 1 只——**最晚 08-03 必须到货**。"

**可视化**：ECharts 阶梯水位图（断料风险 TOP5）+ 安全线虚线 + 红色断料点标记；全量预测表格三档灯：🔴 会断料（预测最低<0）/ 🟡 低于安全线（0~3）/ 🟢 安全。

**数据闭环**：这不是静态报表——对话出库、拍照入库、批量出入库每一笔操作都实时改变水位曲线，预测永远基于最新库存。同一套联动数据还驱动三角色简报（管理员看告急、队长看节点风险、分析员看呆滞）与 AI 邮件助手（催货邮件自动引用断料日期施压）。
