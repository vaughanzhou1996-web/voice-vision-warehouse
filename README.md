...
我是一名国外商学院毕业的留学生，完全不懂代码。我在上海的一家船舶建造 EPC 公司工作，主要负责处理公司的船检文件。对 AI 的兴趣让我尝试把智能体嵌入日常重复性较高的工作中。在运用智能体提高办公效率的同时，我观察到公司仓库管理系统十分老旧——仓库管理员（曹姐）还在填纸质表格，配合 Excel 统计数据；船舶配件数量繁多、型号复杂（经常有十多位大小写字母+数字的组合），她经常加班核对却依然容易出错。昏暗的仓库里操作库存需要带手套，无法使用智能设备。看到这些痛点，我使用 AI 编程工具（Qoder + 通义千问）手搓了这套基于SQL的智能库存管理系统，完整发挥了先进模型的多模态能力，全部功能由 AI 编程完成，并获得相关认可，已在公司真实上线使用。因此，这是一套生产价值已被验证的系统。
## 七大 AI 能力
