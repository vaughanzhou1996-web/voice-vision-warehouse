...
async function speechToText(audio, mimeType = 'audio/webm') {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      const resp = await axios.post(`${BASE_URL}/audio/transcriptions`, {
        model: ASR_MODEL,
        audio: { data: audio, format: mimeType.replace('audio/', '') },
        language: 'zh'
      }, {
        headers: { 'Authorization': `Bearer ${apiKey}`, 'Content-Type': 'application/json' },
        timeout: 30000,
  ...
  throw new Error('语音识别失败: ' + (lastError?.response?.data?.error?.message || lastError?.message));
}
